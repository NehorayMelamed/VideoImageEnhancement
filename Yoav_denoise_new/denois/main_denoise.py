...
def numpy_to_torch(input_image, device=0, flag_unsqueeze=False):
...
def bicubic_interpolate(input_image, X, Y):
    ...
    # tweets_per_accounts = tweets_per_accounts.clamp(zero, 1.0)  #TODO: why clamp?!?!!?
...
class Warp_Object(torch.nn.Module):
    ...
    # Elementwise sum the tweets_per_accounts of a submodule to its input
...
def get_optical_flow_and_occlusion_on_video(input, flow_model=None, occ_model=None):
    ...
    with torch.no_grad():
        for i in range(num_frames - 1):
            ...
            ### Occlusion model tweets_per_accounts: ###
...
def main_denoise(window_size_temporal, stride, video_path=None, base_directory_of_cropped_images=None,
    ...
    for i in range(0, loop_over - 1 - window_size_temporal, stride):
        print(f"Processing - {i} from total {loop_over - 1 - window_size_temporal} ")
        torch.cuda.empty_cache()
...
# Main is here below
if __name__ == "__main__":
    main_denoise(use_roi=False, window_size_temporal=4, stride=1,
