...
__global__ void ecc_reduction_ker(ECC_inputs_ptr<TP> inputs_ptr,
  ...
  if (lane < 8) {                              // we cant use vectorzied load here because the H matrix is not aligned so insted we shuffle within the warp
  ...
  for (auto i = threadIdx.x; i < input_size[1]; i += blockDim.x) {
    ...
    Gt += G * inputs_ptr.current_level_reference_tensor_zero_mean[0];
    Gw += G * inputs_ptr.current_level_input_tensor_warped[0];
...
// at this point we have the Gt, Gw and C matrix for this block, we need to reduce them to a single value
