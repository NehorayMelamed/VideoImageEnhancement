                                  ...
                                  const std::array<int64_t, 2> input_size)
{
  ...
  TP clatw_norm = 0;       // this is the norm of current_level_input_tensor_warped stored in the thread local memory
  TP clatw_dot_clrtzm = 0; // this is current_level_input_tensor_warped * current_level_reference_tensor_zero_mean stored in the thread local memory
  ...
  // allocte shared memory elements
  // static __shared__ Mat<TP, 8> C_shared; since we are going to LDL decomposition we dont need to store the full C matrix in shared memory just the diagonal and the upper triangular part
  static __shared__ Mat_Sym<TP, 8> C_shared;   // this is the shared memory version of the C matrix that we will use to compute the LDL decomposition
  static __shared__ Mat_Sym<TP, 8> LDL_shared; // this is the shared memory version of the LDL decomposition where the diagonal is stored in the diagonal and the upper triangular part is stored in the upper triangular part
  ...
  for (auto i = threadIdx.x; i < input_size[1]; i += blockDim.x)
  {
    ...
    clatw_norm += current_level_input_tensor_warped * current_level_input_tensor_warped;

    // add the current_level_reference_tensor_zero_warped *current_level_reference_tensor_zero_mean to the clatw_dot_clatw
    clatw_dot_clatw += current_level_input_tensor_warped * current_level_reference_tensor_zero_mean;
  ...
  // reduce the clatw_norm and clatw_dot_clatw and store them locally in thread 0 as they are used only once
  clatw_norm = blockReduceSum(clatw_norm);
  clatw_dot_clatw = blockReduceSum(clatw_dot_clatw);
...
#pragma unroll
  ...
  // we now have to invert the C matrix, we do this by using the LDL decomposition

#pragma unroll
  for (int j = 0; j < 8; j++) // we will use wikipedia's algorithm for the LDL decomposition
  {                           // iterate over the rows
    // we first compute the diagonal element where each element in the summation is done in parallel
    TP Dj_summation = 0;
    if (threadIdx.x < j) //
    {
      diag_sum = LDL_shared(threadIdx.x, threadIdx.x) * C_shared(n_row, threadIdx.x) * C_shared(n_row, threadIdx.x);
    }
    // we now warp reduce the diag_sum
    diag_sum = warpReduceSum(diag_sum); // this is only stored in the first thread of the warp
    if (threadIdx.x == 0)
    {
      LDL_shared(j,j) = C_shared(j,j) - diag_sum;
    }

    // we now compute the off diagonal elements where each element in the summation is done in parallel one thred per element
    if (threadIdx.x < j)
    {
      TP off_diag_sum = 0;
      if (threadIdx.x < j)
      {
        off_diag_sum = LDL_shared(threadIdx.x, threadIdx.x) * C_shared(n_row, threadIdx.x) * C_shared(n_row, j);
      }
      off_diag_sum = warpReduceSum(off_diag_sum);
      if (threadIdx.x == 0)
      {
        LDL_shared(j, n_row) = (C_shared(j, n_row) - off_diag_sum) / LDL_shared(j, j);
      }
    }
