...
torch.cuda.synchronize()
end_time = time.time()
t1 = (end_time-start_time)/2
print("original ", t1)


delta_p2 = calc_delta_p_v4.ecc_calc_delta_p(H_matrix,
                                            current_level_reference_tensor_zero_mean,
                                            current_level_input_tensor_warped,
                                            Jx_chosen_values, Jy_chosen_values,
                                            gx_chosen_values, gy_chosen_values)
...
plt.plot(delta_p1.flatten().cpu().numpy(),'r--')
plt.plot(delta_p2.flatten().cpu().numpy(),'g-.')
plt.show()

print(delta_p2- delta_p1)
print("foo")
