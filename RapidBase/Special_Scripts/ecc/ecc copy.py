...
Jxx_prime = torch.randn(T, 1, 1, In_sz,dtype=torch.float).cuda() ## It faster to have these as tensors g, g_c J, J_jacob, such that the last index is the x,y etc. to achive localaity 
...
current_level_reference_tensor_zero_mean = torch.randn(T, 1, 1, In_sz,dtype=torch.float).cuda()
current_level_input_tensor_warped = torch.randn(T, 1, 1, In_sz,dtype=torch.float).cuda()
Gt = torch.zeros((T, 8, 1),dtype=torch.float).to(current_level_reference_tensor_zero_mean.device)
Gw = torch.zeros((T, 8, 1),dtype=torch.float).to(current_level_reference_tensor_zero_mean.device)
