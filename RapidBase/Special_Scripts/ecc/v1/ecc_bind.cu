...
void ecc_reduction_impl(std::array<torch::Tensor, 10> inputs,
  ...
  // std::array<thrust::device_ptr<TP>, 10> inputs_ptr; // an array of thurst device pointers for the inputs
  // std::transform(inputs.cbegin(), inputs.cend(), inputs_ptr.begin(),
  //  [](torch::Tensor tensor) { return thrust::device_pointer_cast(tensor.data_ptr<TP>()); }); // extract the pointers of the inputs
  ...
  output_ZipIterator<TP> output_zip_iter = make_output_zip_itereator<TP>(outputs);
  ...
  thrust::reduce_by_key(keys_iter, keys_iter+total_input_num, input_iter ,thrust::make_discard_iterator() , output_iter, binary_pred, binary_op);
};
