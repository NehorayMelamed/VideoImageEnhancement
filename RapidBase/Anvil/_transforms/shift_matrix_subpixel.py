...
def _shift_matrix_subpixel_fft_batch(matrix: torch.Tensor, shift_H: Tensor, shift_W: Tensor, matrix_fft: Tensor = None, warp_method: str = 'fft') -> torch.Tensor:
    """
    matrix of t, h, w, shifts of size t
    ...
    dims = matrix.dim()
    if dims == 2:
        original_image_displaced, original_image_displaced_raw = _shift_matrix_subpixel_fft(matrix, shift_H, shift_W, matrix_fft, warp_method)
        return original_image_displaced.unsqueeze(0), original_image_displaced_raw.unsqueeze(0)
    ...
    if shift_W.dim() == 1:
        shift_W = shift_W.unsqueeze(1).unsqueeze(1).to(matrix.device)
    if shift_H.dim() == 1:
        shift_H = shift_H.unsqueeze(1).unsqueeze(1).to(matrix.device)
    ...
    #shift_W, shift_H = expand_shifts(matrix, shift_H, shift_W)
    ### Displace input image: ###
    displacement_matrix = torch.exp(-(1j * 2 * torch.pi * ky.repeat(t, 1, 1).to(matrix.device) * shift_H + 1j * 2 * torch.pi * kx.repeat(t, 1, 1).to(matrix.device) * shift_W)).to(matrix.device)
    fft_image = faster_fft(matrix, matrix_fft, dim=[-1, -2])
    fft_image_displaced = fft_image * displacement_matrix
    original_image_displaced_raw = torch.fft.ifftn(fft_image_displaced, dim=[-1, -2])
    original_image_displaced = original_image_displaced_raw.abs()
    ...
    return original_image_displaced, original_image_displaced_raw


def _shift_matrix_subpixel_fft_batch_with_channels(matrix: torch.Tensor, shift_H: Tensor, shift_W: Tensor, matrix_fft: Tensor = None, warp_method: str = 'fft') -> torch.Tensor:
    """
    ...
    #shift_W, shift_H = expand_shifts(matrix, shift_H, shift_W)
    ### Displace input image: ###
    displacement_matrix = torch.exp(-(1j * 2 * torch.pi * ky.repeat(t, c, 1, 1).to(matrix.device) * shift_H + 1j * 2 * torch.pi * kx.repeat(t, c, 1, 1).to(matrix.device) * shift_W)).to(matrix.device)
    fft_image = faster_fft(matrix, matrix_fft, dim=[-1, -2])
    fft_image_displaced = fft_image * displacement_matrix
    original_image_displaced = torch.fft.ifftn(fft_image_displaced, dim=[-1, -2]).abs()
...
# img = read_image_default_torch()
# img = img[0, 0, :64, :64]
# imshow_torch(img)
# imgs = torch.tensor(img).unsqueeze(0).repeat(5, 1, 1)
# shifts = torch.tensor([1.0, 3.0, -5.0, 1, 4])
# # displaced = _shift_matrix_subpixel_fft(imgs, 2.0, 0.0)
# displaced = _shift_matrix_subpixel_fft_batch(imgs, shifts, 0.0)
