...
def extend_vector_length_n(vector: Tensor, N: int) -> Tensor:
    ...
    if vector_length == 1:
        return torch.ones(N).to(vector.device) * vector
...
# TODO elisheva changed it since in former version it deleted C dimension. Now it keeps it as 1
def RGB2Gray(in_rgb: Tensor):
    if len(in_rgb.shape) == 3:
        out_y = 0.299 * in_rgb[0:1, :, :] + 0.587 * in_rgb[1:2, :, :] + 0.114 * in_rgb[2:, :, :]
    elif len(in_rgb.shape) == 4:
        out_y = 0.299 * in_rgb[:, 0:1, :, :] + 0.587 * in_rgb[:, 1:2, :, :] + 0.114 * in_rgb[:, 2:, :, :]
    elif len(in_rgb.shape) == 5:
        out_y = 0.299 * in_rgb[:, :, 0:1, :, :] + 0.587 * in_rgb[:, :, 1:2, :, :] + 0.114 * in_rgb[:, :, 2:, :, :]
    return out_y


def Gray2RGB(input_image: Tensor) -> Tensor:
    if len(input_image.shape) == 2:
        RGB_image = input_image.unsqueeze(0)
        RGB_image = torch.cat([RGB_image, RGB_image, RGB_image], 0)
        return RGB_image

    elif 3 <= len(input_image.shape):
        if input_image.shape[-3] == 1:
            return torch.cat([input_image, input_image, input_image], dim=-3)
        elif input_image.shape[-3] == 3:
            return input_image
        else:
            raise RuntimeError("Input image is not grayscale or RGB")


def RGB2BW_torch(input_image):
    if len(input_image.shape) == 3:
        if input_image.shape[0] == 3:
            input_image = input_image[0:1, :, :]*0.299 + input_image[1:2, :, :]*0.587 + input_image[2:3, :, :]*0.114
