...
def Align_TorchBatch_ToCenterFrame_NormalizedCrossCorrelation(input_tensor, correlation_size=None):
    #TODO: make sure it can accept all the different inputs
    ...
    # input tensor [T,H,W]
    T, C, H, W = input_tensor.shape
    if not correlation_size:
        cross_correlation_shape = 7
        midpoints = (np.fix(H / 2), np.fix(W / 2))
    ...
    # output_CC = torch.cat()
    #########################################################################################################
    corelation_size_plus_one = correlation_size + 1
    ### Get Correct Max Indices For Fit: ###
    ...
    output_CC_flattened_indices = torch.argmax(output_CC.contiguous().view(T, correlation_size * correlation_size)[:, corelation_size_plus_one:-corelation_size_plus_one], dim=-1).unsqueeze(-1) + corelation_size_plus_one
    ...
    i0_original = i0 + 0
    # i0 = torch.min(torch.max(i0 + 0,1),correlation_size-2)
    # i1 = torch.min(torch.max(i1 + 0,1),correlation_size-2)
    i1_original = i1 + 0
    i0_minus1 = i0 - 1
    i0_plus1 = i0 + 1
    i1_minus1 = i1 - 1
