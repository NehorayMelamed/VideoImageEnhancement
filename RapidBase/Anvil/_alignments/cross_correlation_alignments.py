...
from _internal_utils.parameter_checking import type_check_parameters, validate_warp_method
from _internal_utils.test_exceptions import raise_if, raise_if_not
from _internal_utils.torch_utils import faster_fft, true_dimensionality, \
    compare_unequal_dimensionality_tensors, construct_tensor, RGB2Gray, Gray2RGB, RGB2BW, BW2RGB, center_logical_mask, \
...
def normalized_cc_shifts_calc(matrix: Union[torch.Tensor, np.array, tuple, list],
    ...
    return construct_tensor(shifts_y), construct_tensor(
        shifts_x), batch_cc  # torch.stack(normalized_cross_correlations)
...
def default_weights_weighted_cc(matrix: Tensor) -> Tensor:
    ...
    _, _, _, _, _, weights = canny_edge_detection_layer.forward(Gray2RGB(matrix_mean.unsqueeze(0)), 3)
...
def align_circular_cc(matrix: Tensor, reference_matrix: Tensor, matrix_fft: Tensor,
    ...
    if crop_warped_matrix:
        ...
        new_h = H - (max_shift_h.abs().int().item() * 2 + 5)
