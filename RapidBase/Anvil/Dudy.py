...
from _transforms.rotate_shift import fft_rotate_shift
from alignments import align_to_reference_frame_circular_cc
from alignments_layers import minSAD_transforms_layer, LinearPolarLayer
...
# Find rotation from homography matrix
def rotation_from_homography_matrix(H_matrix_tensor):
    # returns in rads
    B = H_matrix_tensor.shape[0]
    # p1, p2 are arbitrary
    p1 = Tensor([[1, 1, 1]] * B).to(H_matrix_tensor.device)
    p2 = Tensor([[2, 2, 1]] * B).to(H_matrix_tensor.device)

    p1_new = p1.matmul(H_matrix_tensor).squeeze(0)[:, :2]
    p2_new = p2.matmul(H_matrix_tensor).squeeze(0)[:, :2]
    ...
    angles = torch.acos(inner_product / (a_norm * b_norm))
...
def FeaturesAlign_getFeatures_AndHomography_1(input_tensor_1, input_tensor_2, number_of_features=500, snn_matcher_threshold=0.8, input_mask=None, flag_return_warp_error=False):
    ...
    return shift_H, shift_W, rotation_degrees, scale, H_matrix, \
           detected_laf_1, detected_laf_2, \
           descriptors_distance, \
           detected_keypoints_centers_1, detected_keypoints_centers_2, \
           local_descriptors_1, local_descriptors_2, \
           response_function_for_laf_1, response_function_for_laf_2
...
def FeaturesAlign_getFeatures_2(input_tensor_1, input_tensor_2, number_of_features=500, snn_matcher_threshold=0.8, input_mask=None):
    ...
    feature_detector_and_descriptor_module = kornia.feature.GFTTAffNetHardNet(number_of_features).to \
        (input_tensor_1.device)
    ...
    ### Combined Module: ###
    gftt_hardnet_detector_plus_matcher = kornia.feature.LocalFeatureMatcher \
        (local_feature=feature_detector_and_descriptor_module,
...
def Align_FeatureBased_SIFT(input_tensor, reference_tensor):
    # TODO I tried playing with it a little but the results are so off I doubt it would end up super exact
    aligned_frames_list = []
    H_matrix_list = []
    rotation_list_degrees = []
    rotation_list_estimated = []
    T2, C2, H2, W2 = reference_tensor.shape
    T1, C1, H1, W1 = input_tensor.shape

    reference_frame = reference_tensor[:1]
    for i in np.arange(T1):
        print(i)
        current_frame = input_tensor[i:i + 1]
        shift_H, shift_W, rotation_degrees, scale, H_matrix, _, _, _, _, _, _, _, _, _ = \
            FeaturesAlign_getFeatures_AndHomography_1(current_frame, reference_frame,
                                                      number_of_features=1500, snn_matcher_threshold=0.8,
                                                      input_mask=None, flag_return_warp_error=False)

        # current_frame_aligned = kornia.geometry.transform.warp_affine(current_frame, H_matrix[:, :2], dsize=(H1, W1))
        current_frame_aligned = kornia.geometry.transform.warp_perspective(current_frame, H_matrix, dsize=(H1, W1))
        rotation = rotation_from_homography_matrix(H_matrix)
        rotation_rads = rotation_degrees * torch.pi / 180
        current_frame_aligned = affine_warp_matrix(current_frame,
                                                   shifts_y=shift_H*0,
                                                   shifts_x=shift_W*0,
                                                   thetas=-rotation,
                                                   warp_method='bilinear')
        current_frame_aligned = crop_center_torch_batch(current_frame_aligned, (H1, W1))
        aligned_frames_list.append(current_frame_aligned)
        H_matrix_list.append(H_matrix)
        rotation_list_degrees.append(rotation_degrees.item())
        rotation_list_estimated.append(rotation.item())

    aligned_tensor_FeatureAlign = torch.cat(aligned_frames_list, 0).squeeze(0)
    rotation_list_estimated = [d * 180 / torch.pi for d in rotation_list_estimated]
    # print(rotation_list)
    return aligned_tensor_FeatureAlign, H_matrix_list, rotation_list_degrees, rotation_list_estimated
...
def Align_Palantir_CC_Then_Classic_Then_CC(input_tensor, reference_tensor, flag_plot=False):
    ...
    aligned_tensor_after_classic_and_CC, shifts_h, shifts_w, CC_matrix = align_to_reference_frame_circular_cc \
        (matrix=crop_center_torch_batch(aligned_frames_classic_tensor, before_final_CC_ROI),
                                                                                         reference_matrix=crop_center_torch_batch(reference_tensor, before_final_CC_ROI),
                                                                                         matrix_fft=None,
                                                                                         normalize_over_matrix=False,
                                                                                         warp_method='fft',
                                                                                         crop_warped_matrix=False)
...
def Align_Palantir_CC_Then_Classic_Then_minSAD(input_tensor, reference_tensor, flag_plot=False):
    ...
    if flag_plot:
        concat_tensor = torch.cat([input_tensor.cpu(), aligned_tensor.cpu()], dim=-1)
        imshow_torch_video(input_tensor, FPS=50)
        imshow_torch_video(aligned_tensor, FPS=10, frame_stride=1)
        imshow_torch_video(concat_tensor, FPS=50)
        imshow_torch_video(input_tensor - input_tensor[T // 2:T // 2 + 1], FPS=50)
        imshow_torch_video(aligned_tensor - aligned_tensor[T // 2:T // 2 + 1], FPS=50)
        del concat_tensor
    ...
    ### Try Kornia's feature based using SIFT: ###
    aligned_tensor_FeatureAlign, H_matrix_list, rotation_list_degrees, rotation_list_rads = Align_FeatureBased_SIFT(aligned_tensor, reference_tensor)
    aligned_frames_classic_tensor = aligned_tensor_FeatureAlign

    # TODO: use the H_matrix to get the translation and rotations to be able to perform precise fft alignment
    if flag_plot:
        concat_tensor = torch.cat([aligned_tensor, aligned_frames_classic_tensor], dim=-1)
        imshow_torch_video(concat_tensor, FPS=5, frame_stride=1)
        imshow_torch_video(aligned_frames_classic_tensor, FPS=5, frame_stride=1)
        imshow_torch_video(aligned_tensor, FPS=10, frame_stride=1)
        imshow_torch_video(aligned_frames_classic_tensor - aligned_frames_classic_tensor[T // 2:T // 2 + 1], FPS=10, frame_stride=1)
    ####################################################################################
    ...
    binning_size = 4
    aligned_tensor_binned = fast_binning_3D_overlap_flexible_AvgPool2d(aligned_frames_classic_tensor, (binning_size, binning_size), (0, 0))
    reference_tensor_binned = fast_binning_3D_overlap_flexible_AvgPool2d(reference_tensor, (binning_size, binning_size), (0, 0))
    ...
    warped_tensor = crop_torch_batch(warped_tensor, (H, W))
    aligned_tensor_binned = aligned_tensor_binned
    ...
    if flag_plot:
        ...
        plt.show()
        sad_torch_list_2 = (warped_tensor.cpu() - aligned_tensor_binned[T // 2:T // 2 + 1].cpu()).abs().mean \
            ([-1, -2]).squeeze().cpu()
        sad_torch_list_1 = (aligned_tensor_binned.cpu() - aligned_tensor_binned[T // 2:T // 2 + 1].cpu()).abs().mean \
            ([-1, -2]).squeeze()
        string_list = []
        for i in np.arange(len(sad_torch_list_1)):
            string_list.append \
                (decimal_notation(sad_torch_list_1[i].item(), 2) + ' , ' + decimal_notation(sad_torch_list_2[i].item(), 2))
        concat_tensor = torch.cat([aligned_tensor_binned.cpu(), warped_tensor.cpu()], -1)
        concat_aligned_tensor = torch.cat([aligned_tensor_binned.cpu(), aligned_tensor_binned.cpu()], -1)
    ...
    warped_tensor_full_resolution = affine_transform_interpolated(aligned_frames_classic_tensor.unsqueeze(0),
                                                                  ...
                                                                  construct_tensor
                                                                      (-rotational_shifts_full_resolution * np.pi / 180),
    ...
    aligned_tensor_after_CC_classic_minSAD = warped_tensor_full_resolution.cpu()
    ####################################################################################
...
def Align_Palantir_CC_Then_minSADReference(input_tensor, reference_tensor, flag_plot=False):
    ...
    # (2). min-SAD Reference Comparison:
    binning_size = 4
    aligned_tensor_binned = fast_binning_3D_overlap_flexible_AvgPool2d(aligned_tensor, (binning_size, binning_size), (0, 0))
    ...
    if flag_plot:
        ...
        sad_torch_list_1 = (aligned_tensor_binned - aligned_tensor_binned[T // 2:T // 2 + 1]).abs().mean \
            ([-1, -2]).squeeze()
        string_list = []
        for i in np.arange(len(sad_torch_list_1)):
            string_list.append \
...
def Align_Palantir_CC_Then_minSADPairWise(input_tensor, reference_tensor, flag_plot=False):
    ...
    warped_tensor_binned_resolution = warped_tensor_binned_resolution.to(f'cuda:{gpu_idx}')
...
# def Align_Palantir_CC_Then_ECC_Then_CC(input_tensor, reference_tensor, flag_plot=False):
#     # Cross Correlation initial correction:
#     # (1). compare to reference (center) frame:
#     T, C, H, W = input_tensor.shape
#     aligned_tensor, shifts_h, shifts_w, CC_matrix = align_to_reference_frame_circular_cc(matrix=input_tensor,
...
#
#     # Do ECC alignment:
#     aligned_tensor_ECC, H_matrix_list = Align_ECC(input_tensor, reference_tensor)
#     aligned_frames_classic_tensor = aligned_tensor_ECC
#     if flag_plot:
#         imshow_torch_video(aligned_tensor_ECC, FPS=50, frame_stride=5)
#         imshow_torch_video(aligned_tensor_ECC - aligned_tensor_ECC[T // 2:T // 2 + 1], FPS=50,
#                            frame_stride=5)
#
#     # Cross Correlation Again:
#     aligned_frames_classic_tensor = aligned_frames_classic_tensor.to(reference_tensor.device)
...
def Align_Palantir_CC_Then_ECC_Then_CC_Then_minSAD(input_tensor, reference_tensor, flag_plot=False):
    # Cross Correlation initial correction:
    # (1). compare to reference (center) frame:
    T, C, H, W = input_tensor.shape
    aligned_tensor_CC, shifts_h, shifts_w, CC_matrix = align_to_reference_frame_circular_cc(matrix=input_tensor,
                                                                                         reference_matrix=reference_tensor,
                                                                                         matrix_fft=None,
                                                                                         normalize_over_matrix=False,
                                                                                         warp_method='fft',
    ...
    # Present results after cross correlation:
    if flag_plot:
        concat_tensor = torch.cat([input_tensor.cpu(), aligned_tensor_CC.cpu()], dim=-1)
        imshow_torch_video(input_tensor, FPS=50)
        imshow_torch_video(aligned_tensor_CC, FPS=50, frame_stride=5)
        imshow_torch_video(concat_tensor, FPS=50)
        imshow_torch_video(input_tensor - input_tensor[T // 2:T // 2 + 1], FPS=50)
        imshow_torch_video(aligned_tensor_CC - aligned_tensor_CC[T // 2:T // 2 + 1], FPS=50)
        del concat_tensor

    # Free up GPU memory:
    input_tensor = input_tensor.to('cpu')
    aligned_tensor_CC = aligned_tensor_CC.to('cpu')
    del CC_matrix
    torch.cuda.empty_cache()

    # Do ECC alignment:
    aligned_tensor_CC_ECC, H_matrix_list, rotation_rad_list, rotation_degree_list, shift_H_list, shift_W_list = Align_ECC(aligned_tensor_CC, reference_tensor)
    aligned_frames_CC_ECC_tensor = aligned_tensor_CC_ECC.to(reference_tensor.device).float()
    if flag_plot:
        imshow_torch_video(aligned_tensor_CC_ECC, FPS=5, frame_stride=1)
        imshow_torch_video(aligned_tensor_CC_ECC - aligned_tensor_CC_ECC[T // 2:T // 2 + 1], FPS=50,
                           frame_stride=5)

    # Do cross correlation again:
    aligned_tensor_CC_ECC_CC, shifts_h_CC_ECC_CC, shifts_w_CC_ECC_CC, CC_matrix_CC_ECC_CC = \
        align_to_reference_frame_circular_cc(matrix=aligned_frames_CC_ECC_tensor,
                                             reference_matrix=reference_tensor,
                                             matrix_fft=None,
                                             normalize_over_matrix=False,
                                             warp_method='fft',
    ...
    # Do minSAD alignment:
    aligned_frames_CC_ECC_CC_tensor = aligned_tensor_CC_ECC_CC.to(reference_tensor.device).float()

    # (2). min-SAD Reference Comparison:
    binning_size = 4
    aligned_tensor_binned = fast_binning_3D_overlap_flexible_AvgPool2d(aligned_frames_CC_ECC_CC_tensor, (binning_size, binning_size), (0, 0))
    reference_tensor_binned = fast_binning_3D_overlap_flexible_AvgPool2d(reference_tensor, (binning_size, binning_size), (0, 0))
    T, C, H, W = aligned_tensor_binned.shape
    minSAD = minSAD_transforms_layer(1, T, H, W)
    warped_tensor, shifts_h, shifts_w, rotational_shifts, scale_shifts, min_sad_matrix = \
    ...
    if flag_plot:
        ...
        for i in np.arange(len(sad_torch_list_1)):
            string_list.append(
                decimal_notation(sad_torch_list_1[i].item(), 2) + ' , ' + decimal_notation(sad_torch_list_2[i].item(),
                                                                                           2))
        concat_tensor = torch.cat([aligned_tensor_binned, warped_tensor], -1)
        concat_aligned_tensor = torch.cat([aligned_tensor_binned, aligned_tensor_binned], -1)
        imshow_torch_video(concat_tensor, FPS=25, frame_stride=5, video_title_list=string_list)
        imshow_torch_video(concat_tensor - concat_aligned_tensor[T // 2:T // 2 + 1], FPS=25, frame_stride=2,
                           video_title_list=string_list)
    ...
    rotational_shifts_full_resolution = rotational_shifts * binning_size
    scale_shifts_full_resolution = scale_shifts * 1
    warped_tensor_full_resolution = affine_transform_interpolated(aligned_frames_CC_ECC_CC_tensor.unsqueeze(0),
                                                                  construct_tensor(-shifts_h_full_resolution),
                                                                  construct_tensor(-shifts_w_full_resolution),
                                                                  construct_tensor(
                                                                      -rotational_shifts_full_resolution * np.pi / 180),
                                                                  ...
                                                                  expand=False)
    warped_tensor_full_resolution = warped_tensor_full_resolution.squeeze(0)
    H, W = aligned_tensor_CC.shape[-2:]
    warped_tensor_full_resolution = crop_torch_batch(warped_tensor_full_resolution, (H, W))
    aligned_tensor_after_CC_ECC_minSAD = warped_tensor_full_resolution

    # Return results
    aligned_tensor_final = aligned_tensor_after_CC_ECC_minSAD
    return input_tensor, aligned_tensor_CC, aligned_tensor_CC_ECC, aligned_tensor_CC_ECC_CC, aligned_tensor_final
...
def Align_Palantir_CC_Then_ECC_Then_CC(input_tensor, reference_tensor, flag_plot=False):
    ...
    del aligned_frames_CC_ECC_tensor
    del CC_matrix_CC_ECC_CC
    torch.cuda.empty_cache()
    ...
    # Add Gimbaless corretion for thetiny fine details
    # TODO move meshgrid out later
    ...
    shift_H, shift_W, rotation_rads, scale, rotation_degrees = get_homography_from_optical_flow(aligned_tensor_CC_ECC_CC,
    ...
    aligned_tensor_CC_ECC_CC_Gimbaless = shift_matrix_subpixel(aligned_tensor_CC_ECC_CC,
                                                               shift_H=shift_H, shift_W=shift_W, warp_method='fft')

    aligned_tensor_CC_ECC_CC_Gimbaless = rotate_matrix(aligned_tensor_CC_ECC_CC_Gimbaless,
                                                       thetas=rotation_rads, warp_method='fft')

    # gimbaless_layer = Gimbaless_Layer_Torch(block_size=32, overlap_size=31)
    # delta_x, delta_y = gimbaless_layer.forward(aligned_tensor_CC_ECC_CC.unsqueeze(0), reference_tensor.unsqueeze(0))
    # delta_x = delta_x.squeeze(0)
    # delta_y = delta_y.squeeze(0)
    #
    # flow = torch.cat([delta_y, delta_x], dim=1)
    #
    # # warp_OF_object = Warp_Tensors_Layer()
    # # aligned_tensor_CC_ECC_CC_Gimbaless = warp_OF_object.forward(aligned_tensor_CC_ECC_CC, flow)
    #
    # warp_OF_object = Warp_Object_OpticalFlow()
    # aligned_tensor_CC_ECC_CC_Gimbaless = warp_OF_object.forward(aligned_tensor_CC_ECC_CC, flow,
    #                                                             flag_bicubic_or_bilinear='bilinear',
    #                                                             flag_input_type='HW')

    # Return results
    aligned_tensor_final = aligned_tensor_CC_ECC_CC_Gimbaless
    return input_tensor, aligned_tensor_final
    # return input_tensor, aligned_tensor_CC, aligned_tensor_CC_ECC, aligned_tensor_CC_ECC_CC, aligned_tensor_final


def Align_Palantir_CC_Then_LinearPolar_Then_CC(input_tensor, reference_tensor, flag_plot=False):
    # Cross Correlation initial correction:
    # (1). compare to reference (center) frame:
    T, C, H, W = input_tensor.shape
    aligned_tensor, shifts_h, shifts_w, CC_matrix = align_to_reference_frame_circular_cc(matrix=input_tensor,
    ...
    # Do LinearPolar alignment for rotation:
    LinearPolar = LinearPolarLayer(B=1, T=T, C=C, H=new_H, W=new_W, device=f'cuda:{gpu_idx}')
    rotational_shifts = LinearPolar.forward(input_tensor, reference_tensor)
    # convert rotational shifts from degrees to radians
    rotational_shifts_rads = rotational_shifts * np.pi / 180
    shifts_h = Tensor([0] * T)
    shifts_w = Tensor([0] * T)
    scale_shifts = Tensor([1] * T)
    # warp tensor
    aligned_tensor_LP = affine_warp_matrix(input_tensor, -shifts_h, -shifts_w, -rotational_shifts_rads, scale_shifts,
                                           warp_method='bicubic')

    if flag_plot:
        imshow_torch_video(aligned_tensor_LP, FPS=50, frame_stride=5)
        imshow_torch_video(aligned_tensor_LP - aligned_tensor_LP[T // 2:T // 2 + 1], FPS=50,
                           frame_stride=5)

    # Cross Correlation Again:
    aligned_tensor_LP = aligned_tensor_LP.to(reference_tensor.device)
    H3, W3 = aligned_tensor_LP.shape[-2:]
    before_final_CC_ROI = (H3 - 10, W3 - 10)
    aligned_tensor_after_LP_and_CC, shifts_h, shifts_w, CC_matrix = align_to_reference_frame_circular_cc(
        matrix=crop_center_torch_batch(aligned_tensor_LP, before_final_CC_ROI),
        reference_matrix=crop_center_torch_batch(reference_tensor, before_final_CC_ROI),
        matrix_fft=None,
        normalize_over_matrix=False,
        warp_method='fft',
    ...
    # Return results
    aligned_tensor_final = aligned_tensor_after_LP_and_CC
    return input_tensor, aligned_tensor, aligned_tensor_final
...
class Gimbaless_Layer_Torch(nn.Module):
    ...
    def __init__(self, block_size=32, overlap_size=16):
        ...
        self.spatial_lowpass_before_temporal_derivative_filter_x = torch.ones((1, 1, 1, self.params.dif_len))  # Read this from file
        # self.dict_from_matlab = scipy.io.loadmat(  # TODO make sure no need to read this
    ...
    def forward(self, input_tensor, reference_tensor=None, flag_pairwise_or_reference='reference'):
        ...
        pxpy = fast_binning_3D_overlap_flexible_AvgPool2d(px * py, prediction_block_size_tuple, overlap_block_size_tuple)
        px2 = fast_binning_3D_overlap_flexible_AvgPool2d(px ** 2, prediction_block_size_tuple, overlap_block_size_tuple)
        py2 = fast_binning_3D_overlap_flexible_AvgPool2d(py ** 2, prediction_block_size_tuple, overlap_block_size_tuple)
        ...
        # Local Prediction:
        A = (fast_binning_3D_overlap_flexible_AvgPool2d((ABtx * px), prediction_block_size_tuple, overlap_block_size_tuple))
        B = (fast_binning_3D_overlap_flexible_AvgPool2d((ABty * py), prediction_block_size_tuple, overlap_block_size_tuple))
...
def test_get_affine_flow_field_and_get_homography():
    ...
    flow_field_X = R_meshgrid * torch.cos(theta_meshgrid + rotation_degress_tensor) - R_meshgrid * torch.cos \
        (theta_meshgrid)
    flow_field_Y = R_meshgrid * torch.sin(theta_meshgrid + rotation_degress_tensor) - R_meshgrid * torch.sin \
        (theta_meshgrid)
...
def test_use_optical_flow_to_get_homography():
    ...
    # ### Get TVNet Instance: ###
    # # from models.pytorch_tvnet.tvnet import *
    # from models.tvnet_pytorch.model.net.tvnet import *
    # tvnet_dict = EasyDict()
    ...
    input_tensor_gimbaless = fast_binning_3D_overlap_flexible_AvgPool2d(input_tensor, (4, 4), (0, 0)).unsqueeze(0)
    input_tensor_warped_gimbaless = fast_binning_3D_overlap_flexible_AvgPool2d(input_tensor_warped, (4, 4), (0, 0)).unsqueeze(0)
    input_tensor_gimbaless = crop_center_torch_batch(input_tensor_gimbaless, (90, 250))
    input_tensor_warped_gimbaless = crop_center_torch_batch(input_tensor_warped_gimbaless, (90, 250))
    ...
    input_tensor_gimbaless = crop_center_torch_batch(input_tensor_gimbaless, (flow_magnitude.shape[-2], flow_magnitude.shape[-1]))
...
def get_homography_from_optical_flow(input_tensor_1, input_tensor_2, gimbaless_block_size=32, gimbaless_overlap=31, input_meshgrid=None):
    ### Gimbaless As Optical-Flow: ###
    # Anvil vibes. make 5D and memorize
    dimensions_memory = NoMemoryMatrixOrigami(input_tensor_1)
    input_tensor_1 = dimensions_memory.expand_matrix(input_tensor_1)
    input_tensor_2 = dimensions_memory.expand_matrix(input_tensor_2)
    # x_vec = torch.arange(W)
    # y_vec = torch.arange(H)
    # [Y, X] = torch.meshgrid(y_vec, x_vec)
    Y, X = input_meshgrid
    Y = dimensions_memory.expand_matrix(Y.unsqueeze(-3)).squeeze(-3)
    X = dimensions_memory.expand_matrix(X.unsqueeze(-3)).squeeze(-3)
    ...
    B, T, C, H, W = input_tensor_1.shape
    N = 10000 # number of points to match the homography matrix to

    X_centered = X - W / 2
    Y_centered = Y - H / 2
    indices = torch.randperm(H * W)[0:N]
    indices = torch.sort(indices).values
    X_new = X_centered + delta_x.squeeze(-3)
    Y_new = Y_centered + delta_y.squeeze(-3)
    keypoints_1_X = torch.flatten(X_centered, start_dim=-2)[..., indices]
    keypoints_1_Y = torch.flatten(Y_centered, start_dim=-2)[..., indices]
    keypoints_centers_1 = torch.cat([keypoints_1_X.unsqueeze(-1), keypoints_1_Y.unsqueeze(-1)], -1)
    keypoints_2_X = torch.flatten(X_new, start_dim=-2)[..., indices]
    keypoints_2_Y = torch.flatten(Y_new, start_dim=-2)[..., indices]
    keypoints_centers_2 = torch.cat([keypoints_2_X.unsqueeze(-1), keypoints_2_Y.unsqueeze(-1)], -1)
    H_matrix = kornia.geometry.homography.find_homography_dlt(keypoints_centers_1.view(B*T, N, 2).float(),
                                                              keypoints_centers_2.view(B*T, N, 2).float(),
                                                              weights=None)
    shift_H, shift_W, rotation_rads, scale, rotation_degrees = \
        affine_parameters_from_homography_matrix(H_matrix)

    # Anvil vibes - back to original dimensionality
    shift_H = dimensions_memory.squeeze_to_original_dims(shift_H.view(B, T, 1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
    shift_W = dimensions_memory.squeeze_to_original_dims(shift_W.view(B, T, 1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
    rotation_rads = dimensions_memory.squeeze_to_original_dims(rotation_rads.view(B, T, 1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
    scale = dimensions_memory.squeeze_to_original_dims(scale.view(B, T, 1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
    rotation_degrees = dimensions_memory.squeeze_to_original_dims(rotation_degrees.view(B, T, 1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)
...
class Warp_Tensors_Layer(nn.Module):
    ...
    def forward(self, input_tensors, flo):
        """
        Args:
            input_tensors: input images of shape (B, C, H, W)
            flo: flow of shape (B, C, H, W)
        Returns:
            output tensors of shape (B, C, H, W)
        """
        if self.grid is None:
            self.B, self.C, self.H, self.W = input_tensors.shape
            xx = torch.arange(0, self.W).view(1, -1).repeat(self.H, 1)   # H cols of 1->W
            yy = torch.arange(0, self.H).view(-1, 1).repeat(1, self.W)   # W rows of 1->H
            xx = xx.view(1, 1, self.H, self.W).repeat(self.B, 1, 1, 1)   # (B ,1 ,H, W)
            yy = yy.view(1, 1, self.H, self.W).repeat(self.B, 1, 1, 1)   # (B ,1 ,H, W)
            self.grid = torch.cat([xx, yy], 1).to(input_tensors.device)  # (B ,2 ,H, W) : 2 mesh grids of image size
        vgrid = self.grid + flo  # add flow to mesh-grids
