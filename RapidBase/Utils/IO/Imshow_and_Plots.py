...
def plot_segmentation_map_with_legends(input_segmentation_map, classes_list):
    ### Expecting input_image to have values between 0 and len(classes_list)
    fig, ax = plt.subplots()
    ax.imshow(input_segmentation_map, cmap='nipy_spectral', interpolation='nearest')
    colors = plt.cm.nipy_spectral(np.linspace(0, 1, len(classes_list)))
    patches = [mpatches.Patch(color=colors[i], label=classes_list[i]) for i in range(len(classes_list))]
    plt.legend(handles=patches, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)


def plot_segmentation_map_with_legends_and_colormap_dict(input_image, input_segmentation_map_RGB, classes_RGB_dict=None):
    ### Get classes names and colors: ###
    classes_list = [key for key in classes_RGB_dict.keys()]
    colors = [np.array(value) / 255 for value in classes_RGB_dict.values()]

    ### Plot segmentation map: ###
    alpha_composite = (0.5 * input_image.astype(float) + 0.5 * input_segmentation_map_RGB.astype(float)).astype(
        np.uint8)
    fig, ax = plt.subplots()
    # ax.imshow(input_segmentation_map_RGB.astype(np.uint8))
    # ax.imshow(alpha_composite.astype(np.uint8))
    concatenated_image = np.concatenate([input_image, alpha_composite.astype(np.uint8)], 1)
...
def imshow_torch(image: object, flag_colorbar: object = True, title_str: object = '', flag_maximize: object = False) -> object:
    fig = figure()
    # plt.cla()
    plt.clf()
    # plt.close()
...
# def imshow_torch_video(input_tensor, number_of_frames=None, FPS=3, flag_BGR2RGB=True, frame_stride=1, flag_colorbar=False, video_title='', video_title_list=None, colormap=None):
#     #TODO: fix colorbar
...
#     ### Initial Show (here is where the scale is set: ###
#     output_tensor = get_correct_form(input_tensor, 0)
#     if output_tensor.min() == 0 and output_tensor.max() == 0:
#         output_tensor[0, 0, :] = input_tensor.max().item()
#     im = plt.imshow(output_tensor, cmap=colormap)
#     if flag_colorbar:
...
#         # ### STUPID way to try and get rid of not showing anything when first image is all zeros: ###
#         # if output_tensor.min() == 0 and output_tensor.max() == 0:
#         #     output_tensor[0,0, :] = input_tensor.max().item()
#         # if output_tensor.dtype==np.bool_:
...
def imshow_torch_video(input_tensor, number_of_frames=None, FPS=3, flag_BGR2RGB=True, frame_stride=1, flag_colorbar=False, video_title='', video_title_list=None, colormap=None):
    ...
    #
    # Function to get the correct form of the tensor/frame for display
    def get_correct_form(input_tensor, i):
        if is_numpy:
            if shape_len == 4:  # (T,H,W,C)
                if flag_BGR2RGB and C==3:
                    output_tensor = cv2.cvtColor(input_tensor[i], cv2.COLOR_BGR2RGB)
                else:
                    output_tensor = input_tensor[i]
            elif shape_len == 3:  # (T,H,W)
                output_tensor = input_tensor[i]
        else:  # PyTorch tensor
            if shape_len == 4:  # (T,C,H,W)
                if flag_BGR2RGB and C==3:
                    output_tensor = cv2.cvtColor(torch_to_numpy(input_tensor[i]), cv2.COLOR_BGR2RGB)
                else:
                    output_tensor = torch_to_numpy(input_tensor[i])
    ...
    # Determine if the input is a numpy array
...
def imshow_video(input_tensor, number_of_frames=None, FPS=3, flag_BGR2RGB=True, frame_stride=1, flag_colorbar=False, video_title='', video_title_list=None, colormap=None):
    # Function to convert torch tensor to numpy array
    imshow_torch_video(input_tensor, number_of_frames=None, FPS=3, flag_BGR2RGB=True, frame_stride=1, flag_colorbar=False, video_title='', video_title_list=None, colormap=None)
...
def imshow_numpy_list_video(input_list, number_of_frames=None, FPS=3, flag_BGR2RGB=True, frame_stride=1, colormap=None):
...
def plot_torch(input_signal1, input_signal2=None, label_1="", label_2=""):
    ...
    labels = [x for x in [label_1, label_2] if x]
    plt.legend(labels)
    plt.show()
...
def BB_convert_notation_XYXY_to_XYXY_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_to_XYXY_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_to_XYWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_to_XYWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_to_XYWHnormalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_to_XYWHnormalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_normalized_to_XYXY(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_normalized_to_XYXY_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_normalized_to_XYWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_normalized_to_XYWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_normalized_to_XYWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_normalized_to_XYWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_normalized_to_XcYcWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_normalized_to_XcYcWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYXY_normalized_to_XcYcWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYXY_normalized_to_XcYcWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_to_XYXY_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_to_XYXY_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_to_XYWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_to_XYWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_to_XcYcWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_to_XcYcWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_normalized_to_XYXY(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_normalized_to_XYXY_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_normalized_to_XYXY_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_normalized_to_XYXY_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_normalized_to_XYWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_normalized_to_XYWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_normalized_to_XcYcWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_normalized_to_XcYcWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XYWH_normalized_to_XcYcWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XYWH_normalized_to_XcYcWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_to_XYXY_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_to_XYXY_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_to_XYWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_to_XYWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_to_XcYcWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_to_XcYcWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_normalized_to_XYXY(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_normalized_to_XYXY_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_normalized_to_XYXY_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_normalized_to_XYXY_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_normalized_to_XYWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_normalized_to_XYWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_normalized_to_XYWH_normalized(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_normalized_to_XYWH_normalized_tuple, BBs, W_image, H_image)
    return BBs_new
...
def BB_convert_notation_XcYcWH_normalized_to_XcYcWH(BBs, W_image=640, H_image=640):
    #[X0,Y0,X1,Y1]->[X0,Y0,W,H]
    BBs_new = BB_convert_notation_function_wrapper(BB_convert_notation_XcYcWH_normalized_to_XcYcWH_tuple, BBs, W_image, H_image)
    return BBs_new
...
#     return (x0,y0,x0+W,y0+H)
#
# def BB_convert_BB_notation_XYXY_to_XY_C_WH(BB_tuple):
#     x0 = BB_tuple[0]
#     y0 = BB_tuple[1]
#     x1 = BB_tuple[2]
#     y1 = BB_tuple[3]
...
#     return ((x0 + W)/ 2,(y0 + H)/ 2,W,H)
#
# def BB_convert_BB_notation_XY_C_WH_to_XYXY(BB_tuple):
#     x0 = BB_tuple[0]
#     y0 = BB_tuple[1]
#     W = BB_tuple[2]
#     H = BB_tuple[3]
...
def draw_bounding_box_with_label_on_image_XYXY(image_frame, BB_tuple=(0,0,100,100), BB_label='', line_thickness=3, color=(255,255,255), flag_draw_on_same_image=True):
    ...
    image_frame_copy = cv2.rectangle(np.ascontiguousarray(image_frame_copy).astype(np.uint8), (x0, y0), (x0 + W, y0 + H), color, line_thickness)
...
def Plot_BoundingBox_On_Movie(Mov, fps=1000, tit="tit", Res_dir='Res_dir', flag_save_movie=1, trajectory_tuple=[], BoundingBox_PerFrame_list=[]):
    ...
    iax1 = 1
    if flag_save_movie:
        #TODO: i tried coded 0 but didn't work for some reason
