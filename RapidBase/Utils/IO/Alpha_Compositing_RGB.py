...
def Alpha_Composite_Images_In_Random_Place_And_Size(input_image_RGB, drone_image_RGBA, H_size_range=(8, 20), flag_to_BW=False,
    ...
    # TODO: overall divide the code to bw and rgb and make th ewhole code more oragnized

    if flag_restrict_area:
        while ( 0 < H_stop < 1150) and ( 1350 < W_stop < W_image):
            H_start, H_stop = get_random_start_stop_indices_for_crop(H_drone, H_image)
            W_start, W_stop = get_random_start_stop_indices_for_crop(W_drone, W_image)
    ...
    if flag_to_BW:
        ...
        large_empty_image_for_drone_RGB_numpy[H_start:H_stop, W_start:W_stop] = drone_image_RGBA_numpy_final[:, :, :-1]
...
if __name__ == "__main__":
    # global
    ### Get Images: ###
    input_images_folder = r'/media/eylon/Datasets/Object_Deteciton_datasets/VISDrone_Palantir_ObjectDetection/test_image'
    output_data_folder = r'/media/eylon/Datasets/Object_Deteciton_datasets/VISDrone_Palantir_ObjectDetection/test_3'
    alpha_channel_folders_path = r'/media/eylon/Datasets/Object_Deteciton_datasets/VISDrone_Palantir_ObjectDetection/test'
    ...
    Alpha_Composite_Multiple_AddOn_Images_From_Folders_In_Random_Places_And_Size_On_Multiple_Images(input_images_folder,
                                                                                                    alpha_channel_folders_path,
                                                                                                    output_data_folder,
                                                                                                    number_of_add_ons_per_image=(1, 3),
                                                                                                    H_size_range=(15, 30),
                                                                                                    flag_to_BW=False,
