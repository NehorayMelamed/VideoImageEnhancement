...
def video_read_video_to_torch_tensor(input_video_path: str, max_number_of_images=np.inf):
    """
    Note:
        requirements:
        numpy version lower than 1.24,
        see more here --> https://copycode.org/attributeerror-module-numpy-has-no-attribute-float/
    """
    ...
    while video_stream.isOpened():
        ...
        if flag_frame_available and counter < max_number_of_images:
            ### convert numpy array to torch tensor: ###
            current_tensor = torch.tensor(current_frame).permute([2, 0, 1])  # [H,W,C] -> [C,H,W]            if counter == 0:
            current_tensor = current_tensor.unsqueeze(0) #[C,H,W] -> [T,C,H,W]
            if counter == 0:
                final_tensor = current_tensor * 1
            else:
                final_tensor = torch.cat([final_tensor, current_tensor], dim=0)
            print(counter)
            ### Uptick counter: ###
            counter += 1
        else:
...
def video_video_to_images(video_path, output_path, frame_stride=1, flag_2_BW=False, number_of_images_to_save=float("inf")):
    #TODO: add frame stride, and output path to save into a diffrent directory
    ...
    while video_stream.isOpened() and counter < frame_count and number_of_images_saved < number_of_images_to_save:
        # print(int(video_stream.get(cv2.CAP_PROP_POS_FRAMES)))
        if counter % frame_stride == 0:
            ...
            if flag_frame_available:
                ...
                cv2.imwrite(
                    os.path.join(output_path, image_full_filename),
                    current_frame)
                number_of_images_saved += 1
                print(f"saved frame: {counter}, images saved {number_of_images_saved}" )
        else:
            flag_frame_available, current_frame = video_stream.read()
...
def video_create_movie_from_images_in_folder(images_path, frame_rate=25, output_folder=None, movie_name=None, flag_print_image_number=True, string_find=".jpg"):
    files_list = path_get_files_recursively(images_path, string_find)
