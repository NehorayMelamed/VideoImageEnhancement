...
def read_frames_from_binary_file_stream_direct(filepath, number_of_frames_to_read=-1, number_of_frames_to_skip=0, roi=[100,100], utype=np.uint8):
    ### Read Frames: ###
    # roi = [160,640]
    f = open(filepath, 'rb')
    Mov = np.fromfile(f, dtype=utype, count=number_of_frames_to_read * roi[0] * roi[1], offset=number_of_frames_to_skip*roi[0]*roi[1]*2)
...
def read_frames_from_binary_file_stream_direct_torch(filepath, number_of_frames_to_read=-1, number_of_frames_to_skip=0, roi=[100,100], utype=np.uint8):
    ### Read Frames: ###
    # roi = [160,640]
    f = open(filepath, 'rb')
    if utype == np.uint8:
        ...
        offset_factor = 4
    Mov = np.fromfile(f, dtype=utype, count=number_of_frames_to_read * roi[0] * roi[1], offset=number_of_frames_to_skip*roi[0]*roi[1]*offset_factor)
    Movie_len = np.int(len(Mov) / roi[0] / roi[1])
    number_of_elements = Movie_len * roi[0] * roi[1]
    Mov = np.reshape(Mov[0:number_of_elements], [Movie_len, roi[0], roi[1]])
    # Mov = Mov[:,2:,2:]  #Get rid of bad frame

    Mov = torch.tensor(Mov.astype(np.float)).unsqueeze(1).float()
    # bla = scale_array_stretch_hist(bla)
    # imshow_torch_video(bla, FPS=50, frame_stride=5)
    return Mov
