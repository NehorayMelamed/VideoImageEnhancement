...
def get_elements_from_list_by_logical_mask(input_list, logical_mask):
    return [input_list[i] for i in np.arange(len(logical_mask)) if logical_mask[i]]
...
def delete_elements_from_list_by_index(primary_list, indices_to_delete):
    ...
    # Delete elements from the primary list based on the sorted indices
    for index in sorted_indices:
        if 0 <= index < len(primary_list):
            del primary_list[index]

def remove_elements_from_list_by_index(primary_list, indices_to_delete):
    # Sort the indices in reverse order to avoid index shifting during deletion
    sorted_indices = sorted(indices_to_delete, reverse=True)
...
def delete_elements_from_list_by_logical_mask(primary_list, keep_mask):
    # Create a new list containing only the elements at True positions in the mask
    new_list = [element for element, keep in zip(primary_list, keep_mask) if keep]

    # Update the primary list with the new list
    primary_list.clear()
    primary_list.extend(new_list)

def remove_elements_from_list_by_logical_mask(primary_list, keep_mask):
    # Create a new list containing only the elements at True positions in the mask
    new_list = [element for element, keep in zip(primary_list, keep_mask) if keep]
