...
class ImageCropper():
    def __init__(self, source, window_name="image"):
        ...
        elif torch.is_tensor(source) or   type(source) == np.ndarray:
            if len(source.shape) == 3:
                self.image = source  ## [2688, 1520, 3])
        else:
            raise RuntimeError("Failed to init ImgaeCropper ")
        self.mouse_pts = []
    ...
    def get_tensor_crop(self):
        ...
        if len(self.mouse_pts) == 2:
            ...
            #cv2.imwrite("cropped_image.jpg", self.selected_roi)
...
def numpy_to_torch(input_image, device='cpu', flag_unsqueeze=False):
    ...
    if input_image.ndim == 3:
        ...
        input_image = np.transpose(input_image, (0, 1, 4, 2, 3)) #[T,H,W,C] -> [T,C,H,W]
    input_image = torch.from_numpy(input_image.astype(np.float32)).float().to(device) # to float32
    if flag_unsqueeze:
        input_image = input_image.unsqueeze(0)  # [C,H,W] -> [1,C,H,W]
    return input_image
...
def fast_binning_2D_AvgPool2d(input_tensor, binning_size_tuple, overlap_size_tuple):
...
def torch_to_numpy_video_ready(input_tensor, flag_BGR2RGB=False):
    ...
    threshold_at_which_we_say_input_needs_to_be_normalized = 2
    if output_tensor.max() < threshold_at_which_we_say_input_needs_to_be_normalized:
...
def torch_BW_to_numpy_video_ready(input_tensor, flag_BGR2RGB=False):
    (B,T,C,H,W), shape_len, shape_vec = get_full_shape_torch(input_tensor)
    if shape_len == 2:
        #[H,W]
        output_tensor = input_tensor.unsqueeze(-1).cpu().numpy()
    elif shape_len == 3:
        #[C,H,W]
        output_tensor = torch_to_numpy(input_tensor)
    elif shape_len == 4:
        #[T,C,H,W]
        output_tensor = torch_to_numpy(input_tensor)
