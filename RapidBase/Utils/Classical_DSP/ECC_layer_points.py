...
def get_jacobian_for_warp_transform_torch(x_vec, y_vec, H_matrix, transform_string, H, W):
    ...
    if str.lower(transform_string) == 'homography':
        ...
        xy = torch.cat([torch.transpose(torch_flatten_image(Jx, True, 'F'), -1, -2),
                        ...
                        torch.ones((1, x_vec_length * y_vec_length)).to(H_matrix.device)],
                       0)  # TODO: before axis was -1
...
def spatial_interpolation_numpy(input_image, H_matrix, interpolation_method, transform_string, x_vec, y_vec, H, W):
    ...
    xy = np.concatenate([np.transpose(numpy_flatten(xx, True, 'F')), np.transpose(numpy_flatten(yy, True, 'F')),
                         np.ones((1, len(numpy_flatten(yy, True, 'F'))))], 0)
...
def spatial_interpolation_torch(input_image, H_matrix, interpolation_method, transform_string, x_vec, y_vec, H, W):
    ...
    xy = torch.cat([torch.transpose(torch_flatten_image(xx, True, 'F'), -1, -2),
                    ...
                    torch.ones((1, len(torch_flatten_image(yy, True, 'F')))).to(input_image.device)], 0).to(
        input_image.device)
    ...
    out = torch.nn.functional.grid_sample(input_image.unsqueeze(0).unsqueeze(0), bilinear_grid, mode='bicubic').squeeze(
        0).squeeze(0)
...
def image_jacobian_torch(gx, gy, jac, number_of_parameters):
    ...
    ### Get Jacobian of warped image with respect to parameters (chain rule i think): ###
    G = gx * jac[0:h, :] + gy * jac[h:,
                                :]  # TODO: understand if there's a better way then concatenating multiple times and then multiplying and then reshaping!!!
    G = torch_reshape_image(G, (h * w, number_of_parameters),
                            order='F').contiguous()  # TODO: understand what this outputs and maybe we can avoid the torch_reshape_image
...
def ECC_numpy(input_tensor, reference_tensor, number_of_levels, number_of_iterations_per_level, transform_string,
              delta_p_init=None):
    ...
    for level_index in np.arange(1, number_of_levels):
        ...
        input_tensor_output_list[level_index] = cv2.resize(input_tensor_output_list[level_index - 1],
                                                           dsize=(W // 2, H // 2))
        reference_tensor_output_list[level_index] = cv2.resize(reference_tensor_output_list[level_index - 1],
                                                               dsize=(W // 2, H // 2))
    ...
    for level_index in np.arange(number_of_levels, 0,
        ...
        current_level_reference_tensor = current_level_reference_tensor[margin:A - margin, margin:B - margin].astype(
            float)
        ...
        for iteration_index in np.arange(number_of_iterations_per_level):
            ...
            wim = spatial_interpolation_numpy(current_level_input_tensor, H_matrix, 'linear', transform_string, x_vec,
                                              y_vec, H, W)  # inverse(backward) warping
            ...
            ones_map = spatial_interpolation_numpy(np.ones_like(current_level_input_tensor), H_matrix, 'nearest',
            ...
            den = (np.dot(numpy_flatten(tempzm, True, 'F').squeeze(),
                          numpy_flatten(wim, True, 'F').squeeze())) - np.transpose(Gt) @ i_C @ Gw
    ...
    for ii in np.arange(C_input):
        warpedImage[:, :, ii] = spatial_interpolation_numpy(initImage[:, :, ii], final_warp, 'linear', transform_string,
                                                            nx2, ny2, H, W)
    H_matrix = final_warp
...
def ECC_torch(input_tensor, reference_tensor, number_of_levels, number_of_iterations_per_level, transform_string,
              delta_p_init=None):
    ### Initialize Parameters: ###
    break_flag = 0
    transform_string = str.lower(transform_string)
    ...
    for level_index in np.arange(1, number_of_levels):
        ...
        input_tensor_output_list[level_index] = torch.nn.functional.interpolate(
            input_tensor_output_list[level_index - 1], scale_factor=0.5)
        reference_tensor_output_list[level_index] = torch.nn.functional.interpolate(
            reference_tensor_output_list[level_index - 1], scale_factor=0.5)
    ...
    for level_index in np.arange(number_of_levels, 0,
        ...
        current_level_reference_tensor = current_level_reference_tensor[margin:A - margin, margin:B - margin].type(
            torch.float32)

        ### ECC, Forward Additive Algorithm: ###
        for iteration_index in np.arange(number_of_iterations_per_level):
            print('Level: ' + str(level_index) + ', Iteration: ' + str(iteration_index))
            wim = spatial_interpolation_torch(current_level_input_tensor, H_matrix, 'linear', transform_string, x_vec,
                                              y_vec, H, W)  # inverse(backward) warping
            ...
            den = (torch.dot(torch_flatten_image(tempzm, True, 'F').squeeze(),
                             torch_flatten_image(wim, True, 'F').squeeze())) - torch.transpose(Gt, -1, -2) @ i_C @ Gw
            lambda_correction = num / den
            # (2). compute error vector:
            imerror = lambda_correction * tempzm - wim
            # (3). compute the projection of error vector into Jacobian G:
    ...
    for ii in torch.arange(C_input):
        warpedImage[ii, :, :] = spatial_interpolation_torch(initImage[ii, :, :], final_warp, 'linear', transform_string,
                                                            nx2, ny2, H, W)
    H_matrix = final_warp
...
class ECC_Layer_Torch(nn.Module):
    ...
    def __init__(self, reference_tensor, number_of_iterations_per_level, number_of_levels=1,
                 transform_string='homography', delta_p_init=None):
        ...
        ### Initialize H_matrix matrix: ###
        H_matrix, number_of_parameters = self.initialize_H_matrix(delta_p_init=None, transform_string=transform_string,
                                                                  device=self.device)
    ...
    def spatial_interpolation_torch(self, input_image, H_matrix, interpolation_method, transform_string, x_vec, y_vec,
                                    X_mat, Y_mat, H, W):
        # %OUT = spatial_interpolation_numpy(IN, H_matrix, STR, transform_string, x_vec, y_vec)
        # % This function implements the 2D spatial interpolation of image IN
        # %(inverse warping). The coordinates defined by x_vec,y_vec are projected through
        ...
        ### Try Simple Transformation: ###
        xx_new = 2 * (H_matrix[0, 0] * X_mat + H_matrix[0, 1] * Y_mat + H_matrix[0, 2]) / (
                    H_matrix[-1, 0] * X_mat + H_matrix[-1, 1] * Y_mat + H_matrix[-1, 2]) / max(W - 1, 1) - 1
        yy_new = 2 * (H_matrix[1, 0] * X_mat + H_matrix[1, 1] * Y_mat + H_matrix[1, 2]) / (
                    H_matrix[-1, 0] * X_mat + H_matrix[-1, 1] * Y_mat + H_matrix[-1, 2]) / max(H - 1, 1) - 1

        ### Subpixel Interpolation 2: ###
        # xx_new = 2 * xx_new / max(W - 1, 1) - 1
        # yy_new = 2 * yy_new / max(H - 1, 1) - 1
        bilinear_grid = torch.cat([torch_get_4D(xx_new, 'CH'), torch_get_4D(yy_new, 'CH')],
                                  dim=3)  # TODO: get rid of the torch_get_4D and maybe find a faster way then torch.cat()
        out = torch.nn.functional.grid_sample(input_image.unsqueeze(0).unsqueeze(0), bilinear_grid,
                                              mode='bilinear').squeeze(0).squeeze(
            0)  # TODO: get rid of the unsqueeze by being consistent with the format
        #################################################################################################################################
    ...
    def get_jacobian_for_warp_transform_torch(self, x_vec, y_vec, H_matrix, Jx, Jy, J0, J1, transform_string, H, W):
        ...
        if str.lower(transform_string) == 'homography':
            ### New, Better Way: ###
            xy_prime_reshaped_X = (H_matrix[0, 0] * Jx + H_matrix[0, 1] * Jy + H_matrix[0, 2]) / (
                        H_matrix[-1, 0] * Jx + H_matrix[-1, 1] * Jy + H_matrix[-1, 2])
            xy_prime_reshaped_Y = (H_matrix[1, 0] * Jx + H_matrix[1, 1] * Jy + H_matrix[1, 2]) / (
                        H_matrix[-1, 0] * Jx + H_matrix[-1, 1] * Jy + H_matrix[-1, 2])
    ...
    def initialize_things_for_first_run(self, reference_tensor):
        ...
        self.J0_list[0] = 0 * self.Jx_list[
            0]  # could also use zeros_like  #TODO: obviously this, like others, can and should be created beforehand!!!
        ...
        for level_index in np.arange(1, self.number_of_levels):
            ### Interpolate: ###
            self.reference_tensor_output_list[level_index] = torch.nn.functional.interpolate(
                self.reference_tensor_output_list[level_index - 1], scale_factor=0.5)
            ...
            self.J0_list[level_index] = 0 * self.Jx_list[
                level_index]  # could also use zeros_like  #TODO: obviously this, like others, can and should be created beforehand!!!
    ...
    def forward(self, input_tensor, reference_tensor, max_shift_threshold=2e-3, flag_print=False):
        ...
        input_tensor_output_list[
            0] = input_tensor  # TODO: only this and the below lines need to be created at each new forward, all the rest can and should be created beforehand
        ...
        for level_index in np.arange(self.number_of_levels, 0,
            ...
            for iteration_index in np.arange(self.number_of_iterations_per_level):
                if flag_print:
                    print('Level: ' + str(level_index) + ', Iteration: ' + str(iteration_index))
                current_level_input_tensor_warped = self.spatial_interpolation_torch(current_level_input_tensor,
                                                                                     H_matrix, 'linear',
                                                                                     self.transform_string, x_vec,
                                                                                     y_vec, X_mat, Y_mat, H,
                                                                                     W)  # inverse(backward) warping
                ...
                ### Gradient Image interpolation (warped gradients): ###
                vx_warped = self.spatial_interpolation_torch(vx, H_matrix, 'linear', self.transform_string, x_vec,
                                                             y_vec, X_mat, Y_mat, H, W)
                vy_warped = self.spatial_interpolation_torch(vy, H_matrix, 'linear', self.transform_string, x_vec,
                                                             y_vec, X_mat, Y_mat, H, W)

                ### Compute the jacobian of warp transform_string: ###
                J_list = self.get_jacobian_for_warp_transform_torch(x_vec + 1, y_vec + 1, H_matrix, Jx, Jy, J0, J1,
                ...
                num = (torch.linalg.norm(current_level_input_tensor_warped)) ** 2 - torch.transpose(Gw, -1,
                                                                                                    -2) @ i_C @ Gw
                den = (
                                  current_level_input_tensor_warped * current_level_reference_tensor_zero_mean).sum() - torch.transpose(
                    Gt, -1, -2) @ i_C @ Gw
                lambda_correction = num / den
                # (2). compute error vector:
                ...
                flag_end_iterations = (
                            iteration_index == self.number_of_iterations_per_level or flag_delta_p_small_enough)
        ...
        C_input = 1
        self.warped_image = spatial_interpolation_torch(input_tensor[:, :], final_warp, 'linear', self.transform_string,
                                                        nx2, ny2, H, W)
        # for ii in torch.arange(C_input):
...
class ECC_Layer_Torch_Batch(nn.Module):
    ...
    def spatial_interpolation_torch(self, input_image, H_matrix, interpolation_method, transform_string, x_vec, y_vec,
                                    X_mat, Y_mat, H, W, bilinear_grid=None):
        # %OUT = spatial_interpolation_numpy(IN, H_matrix, STR, transform_string, x_vec, y_vec)
        # % This function implements the 2D spatial interpolation of image IN
        # %(inverse warping). The coordinates defined by x_vec,y_vec are projected through
        # % H_matrix thus resulting in new subpixel coordinates. The intensity values in
        ...
        if bilinear_grid is None:
            ...
            denom = (H_matrix_corrected[:, 2:3, 0:1] * X_mat + H_matrix_corrected[:, 2:3,
                                                               1:2] * Y_mat + H_matrix_corrected[:, 2:3, 2:3])
            xx_new = 2 * (H_matrix_corrected[:, 0:1, 0:1] * X_mat + H_matrix_corrected[:, 0:1,
                                                                    1:2] * Y_mat + H_matrix_corrected[:, 0:1,
                                                                                   2:3]) / denom / max(W - 1, 1) - 1
            yy_new = 2 * (H_matrix_corrected[:, 1:2, 0:1] * X_mat + H_matrix_corrected[:, 1:2,
                                                                    1:2] * Y_mat + H_matrix_corrected[:, 1:2,
                                                                                   2:3]) / denom / max(H - 1, 1) - 1
            ### Subpixel Interpolation 2: ###
    ...
    def get_jacobian_for_warp_transform_torch(self, x_vec, y_vec, H_matrix, Jx, Jy, J0, J1, transform_string, H, W):
        ...
        if str.lower(transform_string) == 'homography':
            ...
            H_matrix_corrected = H_matrix.unsqueeze(-1)
            den = (H_matrix_corrected[:, 2:3, 0:1] * Jx + H_matrix_corrected[:, 2:3, 1:2] * Jy + H_matrix_corrected[:,
                                                                                                 2:3,
                                                                                                 2:3])  # TODO: this is used three times here!!!!! calculate once and use it!!!!!
            denom_inverse = 1 / den
            xy_prime_reshaped_X = (H_matrix_corrected[:, 0:1, 0:1] * Jx + H_matrix_corrected[:, 0:1,
                                                                          1:2] * Jy + H_matrix_corrected[:, 0:1,
                                                                                      2:3]) * denom_inverse
            xy_prime_reshaped_Y = (H_matrix_corrected[:, 1:2, 0:1] * Jx + H_matrix_corrected[:, 1:2,
                                                                          1:2] * Jy + H_matrix_corrected[:, 1:2,
                                                                                      2:3]) * denom_inverse
    ...
    def initialize_things_for_first_run(self, input_tensor, reference_tensor):
        ...
        self.x_vec_unsqueezed = self.x_vec_list[0].unsqueeze(0)
        self.y_vec_unsqueezed = self.y_vec_list[0].unsqueeze(-1)
        self.Jx_list[0] = torch.repeat_interleave(self.x_vec_unsqueezed, y_vec_length, 0).unsqueeze(0).unsqueeze(
            0).repeat(T, 1, 1, 1)
        self.Jy_list[0] = torch.repeat_interleave(self.y_vec_unsqueezed, x_vec_length, 1).unsqueeze(0).unsqueeze(
            0).repeat(T, 1, 1, 1)
        self.J0_list[0] = 0 * self.Jx_list[
            0]  # could also use zeros_like  #TODO: obviously this, like others, can and should be created beforehand!!!
        self.J1_list[0] = self.J0_list[0] + 1  # could also use ones_like
        ...
        for level_index in np.arange(1, self.number_of_levels):
            ...
            x_vec_unsqueezed = self.x_vec_list[level_index].unsqueeze(0)
            y_vec_unsqueezed = self.y_vec_list[level_index].unsqueeze(-1)
            self.Jx_list[level_index] = torch.repeat_interleave(x_vec_unsqueezed, y_vec_length, 0).unsqueeze(
                0).unsqueeze(0).repeat(T, 1, 1, 1)
            self.Jy_list[level_index] = torch.repeat_interleave(y_vec_unsqueezed, x_vec_length, 1).unsqueeze(
                0).unsqueeze(0).repeat(T, 1, 1, 1)
            self.J0_list[level_index] = 0 * self.Jx_list[
                level_index]  # could also use zeros_like  #TODO: obviously this, like others, can and should be created beforehand!!!
            self.J1_list[level_index] = self.J0_list[level_index] + 1  # could also use ones_like
    ...
    def forward(self, input_tensor, reference_tensor, max_shift_threshold=2e-3, flag_print=False):
        ...
        for level_index in np.arange(self.number_of_levels, 0,
            ...
            for iteration_index in np.arange(self.number_of_iterations_per_level):
                if flag_print:
                    print('Level: ' + str(level_index) + ', Iteration: ' + str(iteration_index))
                current_level_input_tensor_warped, bilinear_grid = self.spatial_interpolation_torch(
                    current_level_input_tensor, H_matrix, 'linear', self.transform_string, x_vec, y_vec, X_mat, Y_mat,
                    H, W)  # inverse(backward) warping
                ...
                ### Gradient Image interpolation (warped gradients): ###
                vx_warped, bilinear_grid = self.spatial_interpolation_torch(vx, H_matrix, 'linear',
                                                                            self.transform_string, x_vec, y_vec, X_mat,
                                                                            Y_mat, H, W, bilinear_grid)
                vy_warped, bilinear_grid = self.spatial_interpolation_torch(vy, H_matrix, 'linear',
                                                                            self.transform_string, x_vec, y_vec, X_mat,
                                                                            Y_mat, H, W, bilinear_grid)
                ...
                # (1). compute lambda parameter:
                # TODO: maybe the norm of the warped tensor remains approximately the same and i can skip this stage????
                num = (torch.linalg.norm(current_level_input_tensor_warped, dim=(-1, -2))).unsqueeze(
                    -1) ** 2 - torch.transpose(Gw, -1, -2) @ i_C @ Gw
        ...
        H, W = input_tensor.shape[-2:]
        C_input = 1
        self.warped_image, bilinear_grid = self.spatial_interpolation_torch(input_tensor, final_warp, 'linear',
...
class ECC_Layer_Torch_Points(nn.Module):
    ...
    def spatial_interpolation_points_torch(self, chosen_indices, input_image, H_matrix, interpolation_method,
                                           transform_string, x_vec, y_vec, X_mat, Y_mat, H, W, bilinear_grid=None,
                                           X_mat_chosen_values=None, Y_mat_chosen_values=None):
        # %OUT = spatial_interpolation_numpy(IN, H_matrix, STR, transform_string, x_vec, y_vec)
        # % This function implements the 2D spatial interpolation of image IN
        # %(inverse warping). The coordinates defined by x_vec,y_vec are projected through
        # % H_matrix thus resulting in new subpixel coordinates. The intensity values in
        ...
        if bilinear_grid is None:
            ...
            denom = (H_matrix_corrected[:, 2:3, 0:1] * X_mat_chosen_values_corrected + H_matrix_corrected[:, 2:3,
                                                                                       1:2] * Y_mat_chosen_values_corrected + H_matrix_corrected[
                                                                                                                              :,
                                                                                                                              2:3,
                                                                                                                              2:3])
            xx_new = 2 * (H_matrix_corrected[:, 0:1, 0:1] * X_mat_chosen_values_corrected + H_matrix_corrected[:, 0:1,
                                                                                            1:2] * Y_mat_chosen_values_corrected + H_matrix_corrected[
                                                                                                                                   :,
                                                                                                                                   0:1,
                                                                                                                                   2:3]) / denom / max(
                W - 1, 1) - 1
            yy_new = 2 * (H_matrix_corrected[:, 1:2, 0:1] * X_mat_chosen_values_corrected + H_matrix_corrected[:, 1:2,
                                                                                            1:2] * Y_mat_chosen_values_corrected + H_matrix_corrected[
                                                                                                                                   :,
                                                                                                                                   1:2,
                                                                                                                                   2:3]) / denom / max(
    ...
    def get_jacobian_for_warp_transform_points_torch(self, x_vec, y_vec, H_matrix, Jx, Jy, J0, J1, transform_string, H,
                                                     W,
                                                     Jx_chosen_values, Jy_chosen_values, J0_chosen_values,
                                                     J1_chosen_values):
        #     %J = get_jacobian_for_warp_transform_numpy(x_vec, y_vec, H_matrix, transform_string)
        # % This function computes the jacobian J of H_matrix transform with respect
        # % to parameters. In case of homography/euclidean transform, the jacobian depends on
        ...
        if str.lower(transform_string) == 'homography':
            ...
            # TODO: i also think the both H*Jx and H*X_mat can be calculated at the same time when going to CUDA!!!!
            H_matrix_corrected = H_matrix.unsqueeze(-1)
            den = (H_matrix_corrected[:, 2:3, 0:1] * Jx_chosen_values + H_matrix_corrected[:, 2:3,
                                                                        1:2] * Jy_chosen_values + H_matrix_corrected[:,
                                                                                                  2:3,
                                                                                                  2:3])  # TODO: this is used three times here!!!!! calculate once and use it!!!!!
            denom_inverse = 1 / den
            xy_prime_reshaped_X = (H_matrix_corrected[:, 0:1, 0:1] * Jx_chosen_values + H_matrix_corrected[:, 0:1,
                                                                                        1:2] * Jy_chosen_values + H_matrix_corrected[
                                                                                                                  :,
                                                                                                                  0:1,
                                                                                                                  2:3]) * denom_inverse
            xy_prime_reshaped_Y = (H_matrix_corrected[:, 1:2, 0:1] * Jx_chosen_values + H_matrix_corrected[:, 1:2,
                                                                                        1:2] * Jy_chosen_values + H_matrix_corrected[
                                                                                                                  :,
                                                                                                                  1:2,
                                                                                                                  2:3]) * denom_inverse
            ...
            # J_down = torch.cat([J0, Jx, -Jxy_prime, J0, Jy, -Jyy_prime, J0, J1], -1)
            # J = torch.cat([J_up, J_down], 0)
            J_list = [Jx_chosen_values, Jy_chosen_values, J0_chosen_values, J1_chosen_values, Jxx_prime, Jxy_prime,
                      Jyx_prime, Jyy_prime]
    ...
    def forward(self, input_tensor, reference_tensor, max_shift_threshold=2e-3, flag_print=False):
        ...
        for level_index in np.arange(self.number_of_levels, 0,
            ...
            reference_tensor_chosen_values = reference_tensor[
                reference_tensor_gradient_above_mean_logical_mask].unsqueeze(0).unsqueeze(0).unsqueeze(0)
            ...
            for iteration_index in np.arange(self.number_of_iterations_per_level):
                ...
                current_level_input_tensor_warped, bilinear_grid = self.spatial_interpolation_points_torch(
                    chosen_indices, current_level_input_tensor, H_matrix, 'linear',
                ...
                ### Gradient Image interpolation (warped gradients): ###
                vx_warped, bilinear_grid = self.spatial_interpolation_points_torch(chosen_indices, vx, H_matrix,
                                                                                   'linear', self.transform_string,
                                                                                   x_vec, y_vec, X_mat, Y_mat, H, W,
                                                                                   bilinear_grid, X_mat_chosen_values,
                                                                                   Y_mat_chosen_values)
                vy_warped, bilinear_grid = self.spatial_interpolation_points_torch(chosen_indices, vy, H_matrix,
                                                                                   'linear', self.transform_string,
                                                                                   x_vec, y_vec, X_mat, Y_mat, H, W,
                                                                                   bilinear_grid, X_mat_chosen_values,
                                                                                   Y_mat_chosen_values)
...
from Quick_Shot.functional_utils.Elisheva_utils import *
...
class ECC_Layer_Torch_Points_Batch(nn.Module):
    ...
    def spatial_interpolation_points_torch(self, chosen_indices, input_image, H_matrix, interpolation_method,
        ...
        if bilinear_grid is None:
            ...
            yy_new = 2 * (H_matrix_corrected[:, 1:2, 0:1] * X_mat_chosen_values_corrected + H_matrix_corrected[:, 1:2,
                ...
                H - 1, 1) - 1
            ### Subpixel Interpolation 2: ###
            bilinear_grid = torch.cat([xx_new, yy_new],
                                      dim=3)  # TODO: this seems to take time! i should probably move towards self.bilinear_grid...for some reason it didn't show speedup!!!!
    ...
    def initialize_things_for_first_run(self, input_tensor, reference_tensor):
        ...
        self.reference_tensor_chosen_values_list[0] = reference_tensor[
            self.reference_tensor_gradient_above_mean_logical_mask]
        ...
        X_mat_chosen_values = self.X_mat_list[0].squeeze(-1).unsqueeze(1)[
            self.reference_tensor_gradient_above_mean_logical_mask].type(
            self.precision)  # TODO: using logical mask returns all values into a 1D array, but i need [T,N] tensor
        Y_mat_chosen_values = self.Y_mat_list[0].squeeze(-1).unsqueeze(1)[
            self.reference_tensor_gradient_above_mean_logical_mask].type(self.precision)
        ...
        for level_index in np.arange(1, self.number_of_levels):
            ...
            y_vec_unsqueezed = self.y_vec_list[level_index].unsqueeze(-1)
            self.Jx_list[level_index] = torch.repeat_interleave(x_vec_unsqueezed, y_vec_length, 0).unsqueeze(
                0).unsqueeze(0).type(self.precision)
            self.Jy_list[level_index] = torch.repeat_interleave(y_vec_unsqueezed, x_vec_length, 1).unsqueeze(
                0).unsqueeze(0).type(self.precision)
            self.J0_list[level_index] = 0 * self.Jx_list[
                level_index]  # could also use zeros_like  #TODO: obviously this, like others, can and should be created beforehand!!!
            ...
            # reference_tensor_gradient_above_mean_logical_mask = v_total_reference > 9
            self.reference_tensor_gradient_above_mean_logical_mask = v_total_reference > v_total_reference_mean
            self.reference_tensor_chosen_values_list[level_index] = self.reference_tensor_output_list[level_index][
                self.reference_tensor_gradient_above_mean_logical_mask]
            X_mat_chosen_values = self.X_mat_list[level_index].squeeze(-1).unsqueeze(1)[
                self.reference_tensor_gradient_above_mean_logical_mask].type(
                self.precision)  # TODO: using logical mask returns all values into a 1D array, but i need [T,N] tensor
            Y_mat_chosen_values = self.Y_mat_list[level_index].squeeze(-1).unsqueeze(1)[
                self.reference_tensor_gradient_above_mean_logical_mask].type(self.precision)
            Jx_chosen_values = self.Jx_list[level_index][self.reference_tensor_gradient_above_mean_logical_mask].type(
                self.precision)
            Jy_chosen_values = self.Jy_list[level_index][self.reference_tensor_gradient_above_mean_logical_mask].type(
                self.precision)
            J1_chosen_values = self.J1_list[level_index][self.reference_tensor_gradient_above_mean_logical_mask].type(
                self.precision)
            J0_chosen_values = self.J0_list[level_index][self.reference_tensor_gradient_above_mean_logical_mask].type(
                self.precision)
            ### Repeat Values For Parallel Processing: ###   #TODO: understand if there's a better way then this!!! i guess using cuda but still
    ...
    def get_new_reference_tensor_gradient_logical_mask(self, reference_tensor):
        ...
        for level_index in np.arange(1, self.number_of_levels):
            ...
            self.reference_tensor_chosen_values_list[level_index] = self.reference_tensor_output_list[level_index][
                self.reference_tensor_gradient_above_mean_logical_mask]
            X_mat_chosen_values = self.X_mat_list[level_index].squeeze(-1).unsqueeze(1)[
                self.reference_tensor_gradient_above_mean_logical_mask]  # TODO: using logical mask returns all values into a 1D array, but i need [T,N] tensor
            Y_mat_chosen_values = self.Y_mat_list[level_index].squeeze(-1).unsqueeze(1)[
                self.reference_tensor_gradient_above_mean_logical_mask]
    ...
    def forward_iterative(self, input_tensor, reference_tensor, max_shift_threshold=2e-3, flag_print=False,
        ...
        for batch_index in np.arange(number_of_batches):
            ...
            if batch_index == 0:
                ...
                H_matrix_init = delta_p_init_negative[
                                start_index - 1:start_index] @ H_matrix_previous_batch @ CC_delta_p_init
            # gtic()
    ...
    def forward(self, input_tensor, reference_tensor, max_shift_threshold=2e-3, flag_print=False,
        ...
        for level_index in np.arange(self.number_of_levels, 0,
            ...
            for iteration_index in np.arange(self.number_of_iterations_per_level):
                ...
                # TODO: maybe the norm of the warped tensor remains approximately the same and i can skip this stage????
                num = (torch.linalg.norm(current_level_input_tensor_warped, dim=(-1, -2))).unsqueeze(
                    -1) ** 2 - torch.transpose(self.Gw, -1, -2) @ i_C @ self.Gw
                den = (current_level_input_tensor_warped * current_level_reference_tensor_zero_mean).sum(
                    [-1, 2]).unsqueeze(-1) - torch.transpose(self.Gt, -1, -2) @ i_C @ self.Gw
                lambda_correction = (num / den).unsqueeze(-1)
...
def ECC_demo_numpy():
    ...
    ECC_layer_object = ECC_Layer_Torch_Points_Batch(input_tensor, reference_tensor, number_of_iterations_per_level=50,
                                                    number_of_levels=1, transform_string='homography',
                                                    delta_p_init=delta_p_init)
    # # gtic()
    H_matrix, warpedImage = ECC_layer_object.forward(input_tensor, reference_tensor, max_shift_threshold=0,
                                                     flag_print=False)
    # # gtoc()
