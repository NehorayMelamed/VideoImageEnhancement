...
def KeOps_fit_1D(x_points, y_points, x_new_points, alpha_ridge=1):
    # ### Parameters for testing: ### #TODO: delete later:
    # alpha_ridge = 1
    # x_points = torch.rand(N, 1).type(dtype)
    # y_points = x_points + 0.5 * (6 * x_points).sin() + 0.1 * (20 * x_points).sin() + 0.05 * torch.randn(N, 1).type(dtype)
    # x_new_points = torch.linspace(0, 1, 1001).type(dtype)[:, None]
    ##################################################################
    ...
    # between a perfect fit (**alpha** = 0) and a smooth interpolation:
    #(1). Ridge Parameter:
    K_xx = gaussian_kernel(x_points, x_points)
    #(2). Get RBF Kernel:
    a = K_xx.solve(y_points, alpha=alpha_ridge)
    #(3). Interpolate/Extrapolate on new points:
    K_tx = gaussian_kernel(x_new_points, x_points)
    y_new_points_fit = K_tx @ a

    # ### Plot Results: ### #TODO: delete later
    # plt.figure(figsize=(8, 6))
    # plt.scatter(x_points.cpu()[:, 0], y_points.cpu()[:, 0], s=100 / len(x_points))  # Noisy samples
    # plt.plot(x_new_points.cpu().numpy(), y_new_points_fit.cpu().numpy(), "r")
    # plt.axis([0, 1, 0, 1])
    # plt.tight_layout()

    return y_new_points_fit


def KeOps_fit_2D(xy_points, z_points, xy_new_points, alpha_ridge=1):
    #(*). [xy_points] = [N,2]
    #(*). [z_points] = [N,1]
    #(*). [xy_new_points] = [N_new,2]

    # ### Paramters for testing: ### #TODO: delete later
    # alpha_ridge = 10
    # xy_points = torch.rand(N, 2).type(dtype)
    # # Some random-ish 2D signal:
    # z_points = ((xy_points - 0.5) ** 2).sum(1, keepdim=True)
    # z_points[z_points > 0.4 ** 2] = 0
    # z_points[z_points < 0.3 ** 2] = 0
    # z_points[z_points >= 0.3 ** 2] = 1
    # z_points = z_points + 0.05 * torch.randn(N, 1).type(dtype)
    # # Add 25% of outliers:
    # Nout = N // 4
    # z_points[-Nout:] = torch.rand(Nout, 1).type(dtype)
    # # Extrapolate on a uniform sample:
    # X = Y = torch.linspace(0, 1, 101).type(dtype)
    # X, Y = torch.meshgrid(X, Y)
    # xy_new_points = torch.stack((X.contiguous().view(-1), Y.contiguous().view(-1)), dim=1)
    #############################################################################################


    ### Perform Ridge regression : ###
    K_xx = laplacian_kernel(xy_points, xy_points)
    a = K_xx.solve(z_points, alpha=alpha_ridge)
    K_tx = laplacian_kernel(xy_new_points, xy_points)
    z_new_points_fit = K_tx @ a

    # ### Plot Results: ###  #TODO: delete later
    # z_new_points_fit = z_new_points_fit.view(101, 101)
    # plt.figure(figsize=(8, 8))
    # plt.scatter(
    #     xy_points.cpu()[:, 0], xy_points.cpu()[:, 1], c=z_points.cpu().view(-1), s=25000 / len(xy_points), cmap="bwr"
    # )
    # plt.imshow(
    #     z_new_points_fit.cpu().numpy()[::-1, :],
