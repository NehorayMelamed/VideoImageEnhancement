...
import cv2
import torch.fft
# from Scenarios_Classic_And_Neural.Classic_TNR.Classical_TNR_misc import *
...
def estimate_noise_IterativeResidual_step(noisy_frame_current,
    ...
    difference_map_squared_averaged = upsample_layer_difference_manin
###p(average_pooling_layer_difference_map(difference_map_squared).mean(0,False).unsqueeze(0))  #if we input image batch average over frames
...
def estimate_noise_ImagesBatch(noisy_frames,
    ...
    # ### Experimental Part - Deprecated TO BE DELETED: ###
    # sigma_noise = 10
