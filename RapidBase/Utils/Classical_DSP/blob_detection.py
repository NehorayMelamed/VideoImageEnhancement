...
from RapidBase.Special_Scripts.import_scripts import *


#####################################################################################################################################################################
### Connected Components Analysis: ###
def get_connected_components_stats(input_tensor, number_of_iterations=10, number_of_components_tuple=(5,5), label_zero_value=0):
    ...
    blob_HcWcHW_total = torch.ones((1,4)).to(input_tensor)
    ...
    for label_index in np.arange(len(unique_labels)):
        ...
        number_of_components_for_this_value = current_label_logical_mask.float().sum().item()  # TODO: this sum is over all possible images, and i want per image!!!
        number_of_components_list.append((label_value.item(), number_of_components_for_this_value))
        indices_HW = logical_mask_to_indices_torch(current_label_logical_mask)[:, -2:]
        blob_W = indices_HW[:,-1].max() - indices_HW[:,-1].min()
        blob_H = indices_HW[:,-2].max() - indices_HW[:,-2].min()
        blob_W = blob_W.unsqueeze(0).unsqueeze(0)
        blob_H = blob_H.unsqueeze(0).unsqueeze(0)
        blob_HW = torch.cat([blob_W.unsqueeze(0).unsqueeze(0),blob_H.unsqueeze(0).unsqueeze(0)], -1)
        blob_Center_H = indices_HW[:,-2].float().mean().unsqueeze(0).unsqueeze(0)
        blob_Center_W = indices_HW[:,-1].float().mean().unsqueeze(0).unsqueeze(0)
        blob_HcWcHW_current = torch.cat([blob_Center_H, blob_Center_W, blob_W, blob_H], -1)
        if label_value == 0:
            final_CC_logical_mask[current_label_logical_mask] = label_zero_value
        else:
            flag_current_label_allowed = (number_of_components_for_this_value >= number_of_components_tuple[0] and number_of_components_for_this_value < number_of_components_tuple[1])
            ...
            if flag_current_label_allowed:
                ...
                blob_HcWcHW_total = torch.cat([blob_HcWcHW_total, blob_HcWcHW_current], 0)
    blob_HcWcHW_total = blob_HcWcHW_total[1:,:]

    ### Get label with max number of components (besides background): ###
    max_label_number_of_components = -1
    max_label_value = 0
    for label_index in np.arange(1, len(number_of_components_list)):
        current_label_value = number_of_components_list[label_index][0]
        current_label_number_of_components = number_of_components_list[label_index][1]
        if max_label_number_of_components < current_label_number_of_components:
            max_label_number_of_components = current_label_number_of_components
            max_label_value = current_label_value
    max_label_pair = torch.tensor([max_label_value, max_label_number_of_components]).unsqueeze(0).to(input_tensor.device)

    return labels_out, final_CC_logical_mask, blob_HcWcHW_total, max_label_pair
...
def get_connected_components_logical_mask_batch(input_tensor, number_of_iterations=10, number_of_components_tuple=(5,5), label_zero_value=0):
    ...
    final_CC_logical_mask = [get_connected_components_logical_mask(input_tensor[i:i + 1],
                                                                   ...
                                                                   number_of_components_tuple=number_of_components_tuple) for i in np.arange(T)]
    # final_CC_logical_mask = torch.cat(final_CC_logical_mask, 0)
    return final_CC_logical_mask

def get_connected_components_stats_batch(input_tensor, number_of_iterations=10, number_of_components_tuple=(5,5), label_zero_value=0):
    T, C, H, W = input_tensor.shape
    stats = [get_connected_components_stats(input_tensor[i:i + 1],
                                                                   number_of_iterations=number_of_iterations,
                                                                   number_of_components_tuple=number_of_components_tuple) for i in np.arange(T)]
    total_logical_mask = []
    total_blobs_THcWcHW = []
    ...
    for i in np.arange(len(stats)):
        current_labels_out = stats[i][0]
        current_logical_mask = stats[i][1]
        current_blobs_HcWcHW = stats[i][2]
        current_max_label_pair = stats[i][3]
        if current_blobs_HcWcHW.shape[0] > 0:
            current_blobs_THcWcHW = torch.cat([torch.tensor([i]).to(input_tensor.device).unsqueeze(0).repeat(current_blobs_HcWcHW.shape[0],1), current_blobs_HcWcHW], -1)
            total_blobs_THcWcHW.append(current_blobs_THcWcHW)
        total_logical_mask.append(current_logical_mask)
        total_labels_out.append(current_labels_out)
        max_label_pair_list.append(current_max_label_pair)
    total_blobs_THcWcHW = torch.cat(total_blobs_THcWcHW)
    total_logical_mask = torch.cat(total_logical_mask)
    total_labels_out = torch.cat(total_labels_out)
    # final_CC_logical_mask = torch.cat(final_CC_logical_mask, 0)
    return total_labels_out, total_logical_mask, total_blobs_THcWcHW, max_label_pair_list
...
def mask_according_to_connected_components_stats_batch(input_tensor, number_of_iterations=10, number_of_components_tuple=(5,5), label_zero_value=0):
    total_labels_out, total_logical_mask, total_blobs_THcWcHW, max_label_pair_list = get_connected_components_stats_batch(input_tensor,
                                                               number_of_iterations=number_of_iterations,
                                                               number_of_components_tuple=number_of_components_tuple)
    input_tensor = input_tensor * total_logical_mask
    return input_tensor, total_labels_out, total_logical_mask, total_blobs_THcWcHW, max_label_pair_list
#####################################################################################################################################################################
...
class Blob_Detection_Layer(torch.nn.Module):
    ...
    def forward(self, input_tensor, flag_plot_BB_on_output_tensor=True):
        ...
        if flag_plot_BB_on_output_tensor:
            output_tensor_with_BB = kornia.utils.draw_rectangle(input_tensor, BB_tensor, color=torch.tensor([1]))
        else:
        ...
        for batch_index in np.arange(input_tensor.shape[0]):
            ...
            for frame_BB_index in np.arange(len(current_BBs)):
                ...
                current_BB_CC_logical_mask = get_connected_components_logical_mask(current_frame_sub_frame,
...
def detect_blobs_torch(input_tensor, filters_response_tensor, sigmas_list, maxpool_spatial_kernel_size=3, detection_threshold=0.03, iou_threshold_modified=0.05):
    ...
    #(4). NMS:
    boxes_tensor_NMS = torch.cat([BB_indices_vec_H_start, BB_indices_vec_H_stop, BB_indices_vec_W_start, BB_indices_vec_W_stop], -1)
...
def blob_detection_center(outliers: Tensor) -> Tensor:
    """
    Returns all Outliers in N,3 form with COM compressed to single middle pixel
    :param outliers: T,H,W boolean Tensor of Outliers
    :return:
    """
    if outliers.dtype != torch.bool:
        outliers = outliers.to(torch.bool)
    outliers = outliers.contiguous()

    blobs_assigned = CCL.CCL(outliers)

    xsums_ysums_numPoints = CCL.CC_Blob_Centers_Formatted(blobs_assigned, 0.9)

    # torch.save(xsums_ysums_numPoints, "tmp_less.pt")
    xsums_ysums_numPoints = xsums_ysums_numPoints[xsums_ysums_numPoints[:, 3].nonzero()].squeeze(1)
    ret = torch.zeros((xsums_ysums_numPoints.shape[0], 3), dtype=torch.int32)
    ret[:, 0] = xsums_ysums_numPoints[:, 0]
    ret[:, 1] = torch.div(xsums_ysums_numPoints[:, 1], xsums_ysums_numPoints[:, 3], rounding_mode='floor')
    ret[:, 2] = torch.div(xsums_ysums_numPoints[:, 2], xsums_ysums_numPoints[:, 3], rounding_mode='floor')
    return ret


def blob_detection_bb(outliers: Tensor) -> Tensor:
    """
    Returns all outliers in N,3 form with COM compressed to single middle pixel
    :param outliers: T,H,W boolean Tensor of outliers
    :return:
    ...
    if outliers.dtype != torch.bool:
        outliers = outliers.to(torch.bool)
    outliers = outliers.squeeze(1)
    blobs_assigned = CCL.CCL(outliers)
    c, h, w, = blobs_assigned.shape
    xsums_ysums_numPoints = CCL.CC_Blob_Bounding_Box(blobs_assigned, 0.9)
    xsums_ysums_numPoints = xsums_ysums_numPoints[xsums_ysums_numPoints[:, 3].nonzero()].squeeze()
    ret = torch.zeros((xsums_ysums_numPoints.shape[0], 5), dtype=torch.int32)
    # ret[:, 0] = xsums_ysums_numPoints[:, 1]
    # ret[:, 1] = h - xsums_ysums_numPoints[:, 3]
    # ret[:, 2] = xsums_ysums_numPoints[:, 2]
    # ret[:, 3] = h - xsums_ysums_numPoints[:, 4]
    ret[:, 0] = xsums_ysums_numPoints[:, 0]
    ret[:, 1] = xsums_ysums_numPoints[:, 1]
    ret[:, 2] = xsums_ysums_numPoints[:, 3]
    ret[:, 3] = xsums_ysums_numPoints[:, 2]
    ret[:, 4] = xsums_ysums_numPoints[:, 4]
